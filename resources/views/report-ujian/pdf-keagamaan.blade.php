...
<head>
    ...
    <title>Laporan Ujian - {{ $siswa->nama }}</title>
    <style>
        body {
        ...
        <thead>
            <tr>
                ...
                {{-- @foreach ($kegiatan as $k)
                    <th style="width: {{ 400 / count($kegiatan) }}px;">{{ $k->nama_kegiatan }}</th>
                @endforeach --}}
            </tr>
        </thead>
        <tbody>
            @forelse($keagamaan as $index => $data)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td class="text-left">{{ $data->nama_kegiatan }}</td>
                    <td class="text-center">{{ $guru[$data->id] }}</td>
                    <td class="text-center">{{ $nilai[$data->id] ?? '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="{{ 3 + count($kegiatan) }}">
                        Tidak ada data
                    </td>
                </tr>
            @endforelse
