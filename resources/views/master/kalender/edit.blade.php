        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">Edit Kalender Akademik</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.kalender.update', $kalender->id) }}" method="POST">
                            ...
                            <select name="tahun_akademik_id" id="tahun_akademik_id"
                                ...
                                @foreach ($tahunAkademikList as $tahun)
                                    <option value="{{ $tahun->id }}"
                                        {{ old('tahun_akademik_id', $kalender->tahun_akademik_id) == $tahun->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenis_libur" id="jenis_libur"
                                ...
                                @foreach ($jenisLiburList as $jenis)
                                    <option value="{{ $jenis }}"
                                        {{ old('jenis_libur', $kalender->jenis_libur) == $jenis ? 'selected' : '' }}>
                                        {{ ucfirst($jenis) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                                rows="3" required>{{ old('keterangan', $kalender->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
